...
grants <- read_csv("data/State_of_Maryland_Grant_and_Loan_Data__FY2009_to_FY2022.csv", guess_max=17765)
```
```{r}
...
A1: **I found 47 total records that fit into Prince Georges school system** 

	
PRINCE GEORGES COUNTY PUBLIC SCHOOL, PRINCE GEORGES PUBLIC SCHOOLS, BOARD OF EDUCATION OF PRINCE GEORGE'S COUNTY, BOARD OF EDUCATION OF PRINCE GEORGE, PRINCE GEORGES COUNTY SCHOOL 

```{r}
pg_schools <- cleaned_payments %>%
   mutate(
    pgcps = case_when(
        str_detect(vendor_name, "PRINCE GEORGES COUNTY PUBLIC SCHOOL") ~ "PGCPS",
        str_detect(vendor_name, "PRINCE GEORGES PUBLIC SCHOOLS") ~ "PGCPS",
        str_detect(vendor_name, "BOARD OF EDUCATION OF PRINCE GEORGE'S COUNTY ") ~ "PGCPS",
        str_detect(vendor_name, " BOARD OF EDUCATION OF PRINCE GEORGE ") ~ "PGCPS",
        str_detect(vendor_name, " PRINCE GEORGES COUNTY SCHOOL") ~ "PGCPS",
    )
   ) %>% 
  filter(!is.na(pgcps))
```
...
A2**2021 had the largest total amount with almost $1.5 Billion. I noticed that there are gaps in the years listed which could be a result of gaps in the data set itself or if I had missed sections that are from PGCPS but were not included within my findings when I looked for the data to include ** 
...
A3.**You will need to clean them so that nearly identical vendors are together. Three examples include: VWR international Inc and VWR international LLC, Becton Dickinson and company and Becton Dickinson, Johnson Controls Inc and Johnson Controls. You could clean this data by adding columns through mutating and using case_when**

```{r} 
capprojumd <- cleaned_payments %>%
...
A4:**In 2009 stem cell research had its highest amount of money invested with 14 million but all other years it has been less then 10 million with the amount in 2022 being only half of 2009 at just over 7 million  ** 

```{r}
stemcellrg <- cleaned_grants %>%
   mutate(
    scrg = case_when(
        str_detect(description, "Stem Cell Research Grant") ~ "SCRG",
        str_detect(description, "STEM CELL RESEARCH GRANT") ~ "SCRG",
    )
   ) %>% 
  filter(!is.na(scrg))
