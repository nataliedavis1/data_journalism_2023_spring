...
**Answer: It tells you different states and their percentage for Trump as well as their percentage of businesseses that recieved ppe and their amount per employee breakdown.**
```{r}
...
**Answer: The data plot is a point for each state with the percentage of businesses recieving ppe on one axis and the percentage for Trump on the other axis. There is a blue line which is the line of best fit, which is showing a positive correlation.  **
```{r}
...
### Task 4: Plot data
**Task** Run the following code. Describe the plot. What do you see?
**Answer: This plot compares the ppe amount per employee with the percentage Trump for each state. The line of best fit is a little more flat as compared to the last plot.**
```{r}
...
**Answer: It shows a data frame that reveals the correlation between percentage Trump and each other category respectively. The correlation is .52 with the businesses receiving ppe and .22 with the amount per employeee.**
...
**Answer: It gives you a number that describes the relationship between the two factors. In this case the correlation variable is .52 which means that there is a positive relationship but it's not the most significant. This output also gives us the information of the p-value which tells us that the relationship is real and not due to random chance. **
```{r}
...
**Task** Run the following code. Describe the model output. What do you see?
**Answer: The output is similar to before, shows a weaker positive relationship with a higher p-value. This .12 p-value indicates that the relationship seems like it happened by chance and is not necessarily a real correlation. **
```{r}
...
**Answer: It is a table with the averages of all the states. It shows that on average the Trump states received more per employee as well as a higher percentage of their businesses recieved PPE loans. **
```{r}
reveal_data %>%
  group_by(vote_2016) %>%
  summarise(
...
**Answer: In this case the p value reveals that the amounts here are not super meaningful and that they are probably more by random chance. **
```{r}
...
### Task 10: Run t-test
**Task** Run the following code. Describe the output. What do you see?
**Answer: In this case the p-value indicates that there is a real connection here, pointing towards a stronger statistical link. **
```{r}
