...
**Answer: The new variable is important because we can use it later down the line in code when we just want to isolate these columns again **
...
**Answer:The dates changed from being in the month/date/year format to now being the year first 2022- then the month and day with dashes and not slashes. For example 3/1/22 changed to 2022-03-01. The data type changed from character to date. **
...
**Answer- The month column contains a year and then a month and then an 01 as the date. This is the computer's way of categorizing the data and sorting it by month within the same year **
...
**Answer: It is making a category that contains the number of total votes and then the percentage of votes can be calculated by dividing out of the total**
...
**Answer: The rows with the highest percentage of Moore votes are at the top and as you go to the next rows the percentage of Moore gets smaller, this is because of the arrange (desc) function **
...
**Answer: It's more interesting to see the counties that have a larger percentage for Moore because with the data set this way we know they don't favor Moore as much but we don't know who they do favor instead without doing the calculations for that **
...
**Answer: str_to_title () converts to title case, where only the first letter of each word is capitalized. str_to_lower () converts everything to lower case  **
...
**Answer: It is extracting the text from the street address and creating a new column with the abbreviation of that state as a new column output. **
...
**Answer: Yes, it does make sense that the total expenses from Maryland would be the highest by a large margin followed by all other states, this is because we are dealing with a data set largely centered around Maryland. **
...
1. What date had the most expenditures occur?

2. Which month had the highest total amounnt of expenditures?

3. What payee spent the most money?
